...
STDAPI NdfCreateSharingIncident(_In_ LPCWSTR UNCPath, _Outptr_ NDFHANDLE* handle);

STDAPI NdfCreateDNSIncident(_In_ LPCWSTR hostname, WORD queryType, _Outptr_ NDFHANDLE* handle);

STDAPI NdfCreateConnectivityIncident(_Outptr_ NDFHANDLE* handle);

STDAPI NdfCreateNetConnectionIncident(_Outptr_ NDFHANDLE* handle, GUID id);

STDAPI NdfCreatePnrpIncident(_In_ LPCWSTR cloudname, _In_opt_ LPCWSTR peername, _In_ BOOL diagnosePublish, _In_opt_ LPCWSTR appId, _Outptr_ NDFHANDLE* handle);
...
NdfDiagnoseIncident(
    ...
    DWORD dwFlags = 0);
#else
NdfDiagnoseIncident(_In_ NDFHANDLE Handle, _Out_ ULONG* RootCauseCount, _Outptr_result_buffer_(*RootCauseCount) RootCauseInfo** RootCauses, DWORD dwWait, DWORD dwFlags);
#endif
