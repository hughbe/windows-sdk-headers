    ...
    void InternalUnlock() throw()
    {
        if (IsLocked())
        {
            ...
            // release the lock
             ...
             // If later classes are added that use non-pointer types for SyncLockT::sync_, this lock
             // annotation will need to be changed.
            _Analysis_assume_lock_held_(*sync_);
...
public:
    ...
    _Acquires_lock_(*cs)
    _Post_same_lock_(*cs, return.sync_)
    ...
    _Acquires_lock_(cs_)
    _Post_same_lock_(cs_, return.sync_)
