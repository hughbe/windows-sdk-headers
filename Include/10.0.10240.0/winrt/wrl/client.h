...
{
protected:
    HRESULT InternalResolve(REFIID riid, _Outptr_result_maybenull_ void** ptr)
    {
        *ptr = nullptr;
        if (ptr_ == nullptr)
        {
            // Agile Reference was already released
            return S_OK;
        }

        HRESULT hr = ptr_->Resolve(riid, ptr);

        if (SUCCEEDED(hr) && *ptr == nullptr)
        {
            // Release the agile reference because it will not succeed ever
            InternalRelease();
        }
...
public:
    ...
    HRESULT AsIID(REFIID riid, _Out_ ComPtr<IUnknown>* ptr) throw()
    {
    ...
    HRESULT CopyTo(REFIID riid, _Outptr_result_maybenull_ IUnknown** ptr) throw()
    {
        __WRL_ASSERT__(riid != __uuidof(IAgileReference));
    ...
    HRESULT CopyTo(_Outptr_result_maybenull_ IAgileReference** ptr) throw()
    {
        InternalAddRef();
        *ptr = ptr_;
        return S_OK;
